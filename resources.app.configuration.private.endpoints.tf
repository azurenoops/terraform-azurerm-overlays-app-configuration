...
resource "azurerm_private_endpoint" "pep" {
  count               = var.enable_private_endpoint && var.existing_private_subnet_name != null ? 1 : 0
  name                = local.app_configuration_pe_name
  location            = local.location
  resource_group_name = local.resource_group_name
  subnet_id           = data.azurerm_subnet.snet.0.id
  tags                = merge({ "Name" = format("%s-private-endpoint", azurerm_app_configuration.app_configuration.name) }, var.add_tags, )

  private_service_connection {
    name                           = "app-config-privatelink"
...
resource "azurerm_private_dns_zone" "dns_zone" {
  ...
  name                = var.environment == "public" ? "privatelink.stores.azure.com" : "privatelink.stores.azure.us"
  resource_group_name = local.resource_group_name
  tags                = merge({ "Name" = format("%s", "Azure-App-Config-Private-DNS-Zone") }, var.add_tags, )
}
...
resource "azurerm_private_dns_a_record" "a_rec" {
  count               = var.enable_private_endpoint ? 1 : 0
  name                = lower(azurerm_app_configuration.app_configuration.name)
